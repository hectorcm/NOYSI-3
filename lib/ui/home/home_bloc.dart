...
class HomeBloC extends BaseBloC with LoadingBloC, ErrorHandlerBloC {
  ...
  final BehaviorSubject<MessageModel?> _pinnedMessageController =
      BehaviorSubject();

  Stream<MessageModel?> get pinnedMessageResult =>
  ...
  Future<void> onJoinedTeamLoadedFromApi(TeamJoinedModel jTeam) async {
    if (waitingForJoinedTeam && jTeam.team.id == lastJoinedTeam) {
      ...
      if (jTeam.team.cname?.enabled == true &&
        ...
        if (jTeam.team.enterprise?.enabled == true &&
            inMemoryData.currentWhiteBrandConfig == null) {
          final wbConfig =
              await _getWhiteBrandConfig(jTeam.team.cname!.domain!);
        ...
        } else if (jTeam.team.enterprise?.enabled == false &&
            inMemoryData.currentWhiteBrandConfig != null) {
  ...
  Future<void> joinTeam() async {
    ...
    var currentTeamId = await _prefs.getStringValue(_prefs.currentTeamId);
    final res =
        await _iTeamRepository.joinTeam(currentTeamName, teamId: currentTeamId);
    if (res is ResultSuccess<TeamJoinedModel>) {
      ...
      if (res.value.team.cname?.enabled == true &&
        ...
        if (res.value.team.enterprise?.enabled == true) {
          final wbConfig =
              await _getWhiteBrandConfig(res.value.team.cname!.domain!);
          await _prefs.setBoolValue(
              _prefs.currentTeamEnterprise, wbConfig?.enabled == true);
          if (wbConfig?.enabled == true) {
            inMemoryData.currentWhiteBrandConfig = wbConfig;
...
//   }
    ...
    if (message.tid == currentTeamId && message.cid == currentChannelId) {
      ...
      if (message.messageStatus == MessageStatus.Updated) {
        ...
        if (currentPinnedMessage != null &&
            currentPinnedMessage.id == message.id &&
            currentPinnedMessage.tid == message.tid &&
            currentPinnedMessage.cid == message.cid) {
        ...
        if (InMemoryData.isInForeground &&
          ...
          if (channelNamePublic != null ||
            ...
            String uidGuide = message.isTaskNotification
                ? (message.args?.uid ?? "")
                : message.uid;
            String sender = uidGuide == RemoteConstants.noysiRobot &&
                    inMemoryData.currentWhiteBrandConfig != null
            ...
            if (shouldSound) {
              AudioPlayer().play(AssetSource(channelNamePublic != null
                  ? R.sound.openChannelsNotification
                  : R.sound.privateGroupsNotification), mode: Platform.isAndroid
  ...
  String _getMessageText(MessageModel messageModel) {
    ...
    if (messageModel.isNoysiWelcomeMessageFirstName) {
      ...
      part2 = messageModel.getChannelMessagePinned(
          Injector.instance.inMemoryData.getMember(messageModel.uid));
    } else if (messageModel.isUnpinnedMessageNotification) {
      part2 = messageModel.getChannelMessageUnpinned(
          Injector.instance.inMemoryData.getMember(messageModel.uid));
  ...
  void onMessageDeleted(MessageDeletedModel messageDeletedModel) async {
    ...
    if (streamData != null &&
        messageDeletedModel.tid == inMemoryData.currentTeam?.id &&
        messageDeletedModel.cid == inMemoryData.currentChannel?.id) {
  ...
  void onChannelUpdated(RTCChannelUpdated model) async {
    ...
    if (idx > -1) {
      ...
      channels[idx].description = model.description;
      channels[idx].callsEnabled = model.allowCalls;
      channels[idx].editMessagesEnabled = model.editingMessagesEnabled;
    }
    ...
    if (chatIndex >= 0) {
      ...
      chats[idx].channelModel?.description = model.description;
      chats[idx].channelModel?.callsEnabled = model.allowCalls;
      chats[idx].channelModel?.editMessagesEnabled =
          model.editingMessagesEnabled;
    }
    if (inMemoryData.currentChannel?.id == model.cid) {
      ...
      inMemoryData.currentChannel?.description = model.description;
      inMemoryData.currentChannel?.editMessagesEnabled =
          model.editingMessagesEnabled;
      inMemoryData.currentChannel?.callsEnabled = model.allowCalls;
    }
          ...
          if (currentIndexRoom >= 0) {
            if (await FlutterAppBadger.isAppBadgeSupported() &&
              ...
              badgeCount = badgeCount -
                          channelList[currentIndexRoom].unreadMessagesCount >=
                      0
                  ? badgeCount -
                      channelList[currentIndexRoom].unreadMessagesCount
                  : 0;
  ...
  void deleteMessage() async {
    ...
    if (currentMessageSelected != null && streamData != null) {
      if (currentMessageSelected?.messageStatus == MessageStatus.Sending) {
        ...
        final currentPinnedMessage = getCurrentPinnedMessage();
        if (currentPinnedMessage != null &&
            currentPinnedMessage.id == currentMessageSelected!.id &&
            currentPinnedMessage.tid == currentMessageSelected!.tid &&
            currentPinnedMessage.cid == currentMessageSelected!.cid) {
          await unpinMessage(currentMessageSelected!);
        }
        _iMessageRepository.deleteMessage(inMemoryData.currentTeam?.id ?? "",
            inMemoryData.currentChannel?.id ?? "", currentMessageSelected!.id);
      }
    }
    currentMessageSelected = null;
  }
  ...
  void downloadFile() async {
    ...
    if (chatRoom != null) {
      ...
      if (res is ResultSuccess<File>) {
        await OpenFilex.open(res.value.path);
      }
  ...
  void doOnUserDeleted(String tid, String uid) async {
    ...
    if (res is ResultSuccess<MemberModel>) {
      ...
      if (tid == await getCurrentTeamId) {
        final idx = joinedTeam?.memberWrapperModel.list
            .indexWhere((element) => element.id == uid);
        if ((idx ?? -1) > -1) {
          joinedTeam?.memberWrapperModel.list[idx!] = res.value;
        }
        final currentChannelList = _drawerChatModelListController.value;
        final mIndex = currentChannelList.indexWhere(
            (element) => element.channelModel?.other == res.value.id);
        if (mIndex > -1) {
  ...
  void onCallingMe(String tid, String cid, String uid) async {
    ...
    callNotificationPlayer.setReleaseMode(ReleaseMode.loop);
    callNotificationPlayer.play(AssetSource(R.sound.callingMe), mode:
    Platform.isAndroid ? PlayerMode.mediaPlayer : PlayerMode.lowLatency);
    txShowCallingNotification(
      ...
      if (accepted) {
        ...
        final res = await joinMeeting(
            room: room, subject: "@${CommonUtils.getMemberUsername(member)}");
        if (res?.isSuccess == true) {
          currentMeeting = MeetingModel(
              url: Injector.instance.meetingBaseUrl,
              room: room,
          ...
          if (resChannel.value.isM1x1) {
            ...
            if (resMember is ResultSuccess<MemberWrapperModel>) {
              ...
              if (member?.id == RemoteConstants.noysiRobot)
                member!.presence = UserPresence.online.name;
              chat = DrawerChatModel(
                  drawerHeaderChatType: DrawerHeaderChatType.Message1x1,
                  isChild: true,
                  isSelected: false,
  ...
  void setUserStatus({String code = '', String text = ''}) async {
    if (code.isEmpty && text.isEmpty)
      ...
      _iUserRepository.updateUserStatus(
          code.replaceAll("-200d-", "-").toUpperCase(), text);
  }
    ...
    if (joinedTeam != null) {
      ...
      if (value.tid == currentTeamId) {
        ...
        if (value.alwaysPush != joinedTeam!.team.alwaysPush) {
          var currentTeamName =
              await _prefs.getStringValue(_prefs.currentTeamName);
          final res = await _iTeamRepository.joinTeam(currentTeamName,
              teamId: currentTeamId, forceApi: true);
          if (res is ResultSuccess<TeamJoinedModel>) {
            waitingForJoinedTeam = true;
        ...
        if (res is ResultSuccess<TeamModel>) {
          _iTeamRepository.joinTeam(res.value.name,
              teamId: value.tid, forceApi: true);
        }
      } else {
        _iTeamRepository.getTeam(value.tid);
      }
  ...
  Future<void> _loadDrawerConfig() async {
    final favoritesExpanded =
        await _prefs.getBoolValue(_prefs.favoritesExpanded, defValue: true);
    final channelsExpanded =
        await _prefs.getBoolValue(_prefs.channelsExpanded, defValue: true);
    final imsExpanded =
        await _prefs.getBoolValue(_prefs.imsExpanded, defValue: true);
    final groupsExpanded =
        await _prefs.getBoolValue(_prefs.groupsExpanded, defValue: true);
    _drawerConfigController.sinkAddSafe(DrawerConfig(
        favoritesExpanded: favoritesExpanded,
        channelExpanded: channelsExpanded,
        imsExpanded: imsExpanded,
  ...
  Future<void> unpinMessage(MessageModel message) async {
    _fullScreenLoadingController.sinkAddSafe(true);
    await _iMessageRepository.unpinMessage(
        message.tid, message.cid, message.id);
    _fullScreenLoadingController.sinkAddSafe(false);
  }

  MessageModel? getCurrentPinnedMessage() =>
      _pinnedMessageController.valueOrNull;
  ...
  void onMessagePinnedUnpinned(RTCMessagePinnedUnpinned model) {
    ...
    if (model.pinnedMessage.tid == currentTeamId &&
        model.pinnedMessage.cid == currentChannelId) {
      _pinnedMessageController
          .sinkAddSafe(model.unpinned ? null : model.pinnedMessage);
    }
  }

  void _navigateToChannelInCurrentTeam(
      String channelName, bool channelIsIMS) async {
    _fullScreenLoadingController.sinkAddSafe(true);
    final currentTeamId = await _prefs.getStringValue(_prefs.currentTeamId);
    ChannelModel? channel;
    MemberModel? other;
    if (channelIsIMS) {
      final resMembers = await _iUserRepository.getTeamMembers(currentTeamId,
          query: channelName.replaceFirst("@", ""),
          max: 1,
          action: RemoteConstants.searchSearch,
          active: false);
      if (resMembers is ResultSuccess<MemberWrapperModel> &&
          resMembers.value.list.isNotEmpty) {
        other = resMembers.value.list.first;
        final resChannel = await _iChannelRepository.create1x1Channel(other,
            tid: currentTeamId);
        if (resChannel is ResultSuccess<ChannelModel>) {
          channel = resChannel.value;
        }
      ...
      final resChannels = await Future.wait([
        _iChannelRepository.getChannels(currentTeamId,
            forceApi: true, type: RemoteConstants.channel),
        _iChannelRepository.getChannels(currentTeamId,
            forceApi: true, type: RemoteConstants.group)
      ]);
      if (resChannels.first is ResultSuccess<List<ChannelModel>> &&
          resChannels.last is ResultSuccess<List<ChannelModel>> &&
          ((resChannels.first as ResultSuccess<List<ChannelModel>>).value +
                  (resChannels.last as ResultSuccess<List<ChannelModel>>).value)
              .isNotEmpty) {
        final channels =
            (resChannels.first as ResultSuccess<List<ChannelModel>>).value +
                (resChannels.last as ResultSuccess<List<ChannelModel>>).value;
        channel =
            channels.firstWhereOrNull((element) => element.name == channelName);
      }
    }
    _fullScreenLoadingController.sinkAddSafe(false);
    if (channel != null && channelIsIMS && other != null) {
      changeChannelAuto(
          ChannelCreatedUI(channelModel: channel, members: [other!]));
    } else if (channel != null && !channelIsIMS) {
      changeChannelAuto(ChannelCreatedUI(channelModel: channel));
    } else {
      showErrorMessageFromString(R.string.channelNotFound);
    }
  }

  void _navigateToChannel(String teamName, String channelName) async {
    final currentTeamName = await _prefs.getStringValue(_prefs.currentTeamName);
    final channelIsIMS = channelName.startsWith("@");
    if (teamName == currentTeamName) {
      _navigateToChannelInCurrentTeam(channelName, channelIsIMS);
    } else {
      _fullScreenLoadingController.sinkAddSafe(true);
      final jTeamRes = await _iTeamRepository.joinTeam(teamName);
      _fullScreenLoadingController.sinkAddSafe(false);
      if (jTeamRes is ResultSuccess<TeamJoinedModel>) {
        changeTeam(jTeamRes.value.team);
        StreamSubscription? ss;
  ...
  void _navigateToTask(String teamName, String taskId) async {
    final currentTeamName = await _prefs.getStringValue(_prefs.currentTeamName);
    if (teamName == currentTeamName) {
      NavigationUtils.push(NoysiApp.navigatorKey.currentContext!,
  ...
  void _navigateToMessageInCurrentTeam(
      String tid, String cid, String mid) async {
    final resChannel = await _iChannelRepository.getChannel(tid, cid);
    ...
    if (resChannel is ResultSuccess<ChannelModel> &&
        resMessage is ResultSuccess<MessageModel>) {
      channel = resChannel.value;
      message = resMessage.value;
      if (channel.isM1x1) {
        final resMembers = await _iUserRepository.getChannelMembers(tid, cid,
            all: true, active: false);
        if (resMembers is ResultSuccess<MemberWrapperModel> &&
            resMembers.value.list.isNotEmpty) {
          other = resMembers.value.list
              .firstWhereOrNull((element) => element.id == channel!.other);
        }
      }
    }
    if (channel != null && message != null && channel.isM1x1 && other != null) {
      changeChannelAuto(ChannelCreatedUI(
          channelModel: channel,
          lastReadMessage: message,
          fromSearchMessage: true,
          members: [other]));
    } else if (channel != null && message != null && !channel.isM1x1) {
      changeChannelAuto(ChannelCreatedUI(
          channelModel: channel,
          lastReadMessage: message,
          fromSearchMessage: true));
  ...
  void _navigateToMessage(String tid, String cid, String mid) async {
    ...
    if (currentTeamId == tid) {
      _navigateToMessageInCurrentTeam(tid, cid, mid);
    } else {
      _fullScreenLoadingController.sinkAddSafe(true);
  ...
  void processNavigationFromLink(AppLinksNavigationModel model) async {
    if (model.link.toLowerCase().trim().startsWith("http") ||
        model.link.toLowerCase().trim().startsWith("com.noysi")) {
      Uri? uri = Uri.tryParse(model.link.trim());
      if (uri != null) {
        if (uri.scheme != "https") {
          uri = uri.replace(scheme: "https");
        }
        if (uri.toString().toLowerCase().startsWith(Endpoint.meetBaseUrlProd) ||
            uri.toString().toLowerCase().startsWith(Endpoint.meetBaseUrlPre) ||
            uri.toString().toLowerCase().startsWith(Endpoint.meetBaseUrlDev)) {
          ...
          ss = joinedTeamLocally.listen((value) async {
            if (value) {
              ...
              final channelRes =
                  await _iChannelRepository.getChannel(teamId, channelId);
              ...
              if (channelRes is ResultSuccess<ChannelModel>) {
                if (channelRes.value.isM1x1) {
                  is1x1 = true;
                  final dynamic userRes =
                      inMemoryData.getMember(channelRes.value.other)?.profile ??
                          await _iUserRepository.getTeamMemberProfile(
                              teamId, channelRes.value.other);
                  if (userRes != null && userRes is MemberProfile) {
                    subject = userRes.name;
                  } else if (userRes != null &&
                      userRes is ResultSuccess<MemberProfile>) {
              ...
              final res =
                  await joinMeeting(url: url, room: room, subject: subject);
              if (res?.isSuccess == true) {
                currentMeeting = MeetingModel(
                    room: room,
                    url: url,
        ...
        } else if (uri.path.toLowerCase().startsWith("/a") &&
            uri.fragment.toLowerCase().contains("/tasks")) {
          ...
          ss = joinedTeamLocally.listen((value) async {
            ...
            uri.fragment.toLowerCase().contains("/messages")) {
          ...
          ss = joinedTeamLocally.listen((value) async {
            ...
            uri.fragment.toLowerCase().contains("/channels") &&
            uri.fragment.toLowerCase().contains("/chat")) {
          final fragments = uri.fragment.split("/");
          final teamName = fragments[2];
          final channelName = fragments[4];
          StreamSubscription? ss;
          ss = joinedTeamLocally.listen((value) async {
            if (value) {
              await NavigationUtils.popUntilWithRouteAndMaterial(
                  NoysiApp.navigatorKey.currentContext!,
                  NavigationUtils.HomeRoute);
              _navigateToChannel(teamName, channelName);
              ss?.cancel();
            }
          });
