...
class _ThreadTypeState extends StateWithBloC<ThreadTypePage, ThreadTypeBloC>
  ...
  StreamSubscription? ssFollow, ssUnfollow;
  ...
  void initState() {
    ...
    ssUnfollow = onThreadUnfollowed.listen((value) {
      if(value.pmid == widget.threadId && bloc.currentUserId == value.uid && bloc.currentTeamId == value.tid) {
        NavigationUtils.pop(context);
      }
    });
    ssFollow = onThreadFollowed.listen((value) {
      if(value.pmid == widget.threadId && bloc.currentUserId == value.uid && bloc.currentTeamId == value.tid) {
        bloc.isBeingFollowed = true;
      }
    });
