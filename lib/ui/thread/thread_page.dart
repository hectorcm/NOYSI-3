...
class _ThreadState extends StateWithBloC<ThreadPage, ThreadBloC> {
  ...
  void initState() {
    ...
    onThreadUnfollowed.listen((value) {
      if (bloc.currentTeamId == value.tid && bloc.currentUserId == value.uid) {
        bloc.removeThread(value.pmid);
      }
    });
    onThreadFollowed.listen((value) {
      if (bloc.currentUserId == value.uid && bloc.currentTeamId == value.tid) {
        bloc.loadThread(value.pmid);
      }
    });
  }
          ...
          actions: [
            PopupMenuButton(
                ...
                itemBuilder: (context) {
                  return [
                    PopupMenuItem(
                      value: ThreadMenu.markAll,
                      child: TXMenuOptionItemWidget(
                        icon: Icon(Icons.check_circle_outline, color: R.color.grayColor),
                        text: R.string.markAllThreads,
                      ),
                    ),
                    PopupMenuItem(
                      value: ThreadMenu.unfollowAll,
                      child: TXMenuOptionItemWidget(
                        icon: Icon(Icons.close, color: R.color.grayColor),
                        text: R.string.unfollowAllThreads,
                      ),
                    )
                  ];
                },
                onSelected: (key) {
                  if(key == ThreadMenu.markAll) {
                     bloc.markAsReadAll();
                  } else {
                    bloc.unfollowAll();
                  }
  ...
  Widget _getThreadWidget(ThreadModel threadModel) {
    ...
    threadModel.childMessages.forEach((element) {
      final member = bloc.members.firstWhereOrNull((m) => m.id == element.uid);
      if (member != null) {
        bool isUnread = false;
        if ((threadModel.tsLastReadByUser
                    ?.compareTo(element.edited ?? element.ts!) ??
                1) <
            0) {
          unread++;
          isUnread = true;
          ...
          children: <Widget>[
            Container(
              margin: EdgeInsets.only(
                  top:
                      member.profile?.fullNameWithUsername.isUpperCase() == true
                          ? 3.5
                          : 5.7),
                ...
                children: <Widget>[
                  ...
                  Container(
                    padding: EdgeInsets.only(
                        bottom: element.isFile ? 0 : 8, left: 8, right: 8),
          ...
          Card(
            child: InkWell(
              onTap: () async {
                ...
                bloc.markAsRead(threadModel.pmid, localOnly: true);
              },
              onLongPress: () {
                            ...
                            children: <Widget>[
                              ...
                              Container(
                                ...
                                child: TXMessageBodyWidget(
                                  message: MessageModel(
                                      text:
                                          threadModel.parentMessage?.text ?? "",
                                      html:
                                          threadModel.parentMessage?.html ?? "",
                                      messageStatus: MessageStatus.Sent),
                                ),
                              ),
                            ],
