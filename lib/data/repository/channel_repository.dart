...
PublishSubject<List<ChannelModel>>? channelsLoadedFromApi;
class ChannelRepository extends BaseRepository implements IChannelRepository {
  ...
  Future<Result<List<ChannelModel>>> getChannels(String teamId,
    ...
    try {
      channelsLoadedFromApi = PublishSubject();
  ...
  @override
  Future<Result<ChannelModel>> updateChannel(String tid, String cid, String name, String description, bool editMessagesEnabled, bool allowCallsEnabled) async {
    try {
      final res = await _channelApi.updateChannel(tid, cid, name, description, editMessagesEnabled, allowCallsEnabled);
      res.tid = tid;
      _iChannelDao.getChannel(cid).then((channel) async {
        if (channel != null) {
          channel.name = res.name!;
          channel.title = res.title!;
          channel.description = res.description;
          channel.editMessagesEnabled = res.editMessagesEnabled;
          channel.callsEnabled = res.callsEnabled;
          await _syncLocalChannel(channel, LocalSyncOperation.Update);
          _inMemoryData.addReplaceChannel(channel);
        }
      });
